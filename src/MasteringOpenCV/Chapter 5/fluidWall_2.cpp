...
static vector<Emitter> emitters(MAX_EMITTERS);
...
/**
 * Loads texture kinect, or webcam and flips depthImage
 * horizontally and vertically. Upon output, depthImage contains user silhouettes.
 * 
 * resizedUsersMatrix = NxN matrix. 
 */
int loadImage()
{
	Mat smallUsersMatrix = Mat::zeros(N, N, CV_8UC1);
	Mat smallDepthMatrix = Mat::zeros(N, N, CV_8UC1);
	...
	// Process depthImage, resize to simulation size.
	resize(depthMatrix, smallDepthMatrix, smallDepthMatrix.size(), 0, 0, INTER_CUBIC);
	depthImage = smallDepthMatrix;

	// Copy filtered depthImage for optical flow to use.
	smallDepthMatrix.copyTo(currFlowImg);

	if(useUserSolver)
	{
		// Process UserID matrix.
		usersMatrix = kinect->getUsersMat();
		resize(usersMatrix, smallUsersMatrix, smallUsersMatrix.size(), 0, 0, INTER_CUBIC);
	}
    return 0;
}

/**
 * Translates depthImage values into boundaries in the FluidSolver. 
 * Currently set to define a boundary at any pixel in the depthImage with a value greater than zero
 */
